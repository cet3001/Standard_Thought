...
const Footer = () => {
  ...
  return (
    <footer className="glass-effect border-t border-accent/20 py-16 urban-texture relative bg-brand-black/50 dark:bg-brand-black">
      {/* Background elements */}
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          ...
          <div className="lg:col-span-2">
            <Link to="/" className="text-3xl font-bold font-satoshi mb-4 block group">
              <span className="group-hover:text-shadow-glow transition-all duration-300 text-brand-cream">Standard</span>
              <span className="text-accent">thought</span>
            </Link>
            <p className="text-brand-cream/70 mb-6 max-w-md leading-relaxed">
            ...
            <div className="flex space-x-4">
              <a 
                ...
                className="w-12 h-12 glass-effect hover:bg-accent text-accent hover:text-black rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110 group bg-white/5"
              >
            ...
            <ul className="space-y-3">
              {footerLinks.navigation.map((link) => (
                <li key={link.path}>
                  <Link 
                    to={link.path} 
                    className="text-brand-cream/70 hover:text-accent transition-all duration-300 relative group"
                  >
                    {link.label}
        ...
        <div className="glass-effect border border-accent/30 rounded-3xl p-8 mb-12 relative overflow-hidden group bg-white/5">
          ...
          <div className="max-w-2xl mx-auto text-center relative z-10">
            <h3 className="text-2xl font-bold mb-2 text-shadow-glow text-brand-cream">Stay Connected</h3>
            ...
            <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
              <input 
                ...
                className="flex-1 px-4 py-3 rounded-2xl border border-accent/30 bg-white/10 focus:outline-none focus:border-accent transition-all duration-300 glass-effect text-brand-cream placeholder:text-brand-cream/50"
              />
        ...
        <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-accent/20">
          <div className="text-brand-cream/60 text-sm mb-4 md:mb-0">
          ...
          <div className="flex space-x-6 text-sm">
            {footerLinks.legal.map((link) => (
              <Link 
                key={link.path}
                to={link.path} 
                className="text-brand-cream/60 hover:text-accent transition-all duration-300 relative group"
              >
                {link.label}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-accent transition-all duration-300 group-hover:w-full"></span>
              </Link>
