...
const InvestingCTA = () => {
  ...
  // Find the investing guide
  const investingGuide = guides?.find(guide => 
    guide.title.toLowerCase().includes('investing action plan') ||
    guide.title.toLowerCase().includes('investment guide') ||
    guide.title.toLowerCase().includes('portfolio builder')
  );

  const handleJoinCommunity = async () => {
    trackButtonClick('Join Community', 'investing_page', 'investing_cta');
    
    if (!investingGuide) {
      toast({
        title: "Guide not found",
        description: "The investing guide is not available right now. Please try again later.",
        variant: "destructive",
      });
      return;
    }
    ...
    const email = prompt('Enter your email to join the community and get the Investing Action Plan:');
    if (!email) return;
    ...
    try {
      ...
      // Download the guide
      await downloadGuide(investingGuide.file_path || investingGuide.file_name || 'Investing Action Plan.pdf', email);
      ...
      <div className="container mx-auto px-6 max-w-4xl relative z-10">
        <div className="bg-gradient-to-r from-[#FFD700]/10 to-[#FFA500]/10 backdrop-blur-sm rounded-2xl p-8 sm:p-12 border-2 border-[#FFD700]/30">
          <div className="text-center">
            ...
            >
              {isSubmitting || isDownloading ? 'Joining Community...' : 'Get More Investing Moves—Join the Community'}
            ...
            <p className="text-sm text-black/70 dark:text-brand-cream/70">
              Get the Investing Action Plan + join our newsletter • No spam, just real blueprints and updates
            </p>
          </div>
        </div>
      </div>
