...
const MobileMenu = () => {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent 
        side="left" 
        className="w-[300px] sm:w-[400px] bg-white border-r border-[#247EFF]/20 relative overflow-hidden z-50"
      ...
      >
        ...
        {/* Strong, contained background - fully opaque */}
        <div className="absolute inset-0 bg-white">
          {/* Subtle urban texture contained within menu */}
          <div className="absolute inset-0 opacity-[0.03] bg-[radial-gradient(circle_at_1px_1px,_rgba(36,126,255,0.4)_1px,_transparent_0)] bg-[length:16px_16px]"></div>
          <div className="absolute inset-0 opacity-[0.02] bg-[linear-gradient(45deg,_transparent_45%,_rgba(36,126,255,0.3)_47%,_rgba(36,126,255,0.3)_53%,_transparent_55%)] bg-[length:12px_12px]"></div>
          <div className="absolute inset-0 bg-gradient-to-b from-[#247EFF]/3 via-transparent via-50% to-[#FFD700]/3"></div>
          
          {/* Corner accents for urban feel */}
          <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-[#247EFF]/8 to-transparent rounded-bl-full"></div>
          <div className="absolute bottom-0 left-0 w-24 h-24 bg-gradient-to-tr from-[#FFD700]/8 to-transparent rounded-tr-full"></div>
        </div>
        ...
        <nav className="flex flex-col space-y-6 mt-12 relative z-20" role="navigation" aria-label="Main navigation">
          ...
          {navItems.map((item, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-[#247EFF]/8 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-lg -mx-2 -my-1"></div>
              <span className="relative z-10">{item.label}</span>
            </Link>
          ))}
          
          {/* Auth Section with enhanced contrast background */}
          <div className="pt-6 border-t border-[#247EFF]/20 relative">
            <div className="absolute inset-0 bg-gradient-to-r from-[#247EFF]/5 to-[#FFD700]/5 rounded-lg -mx-4 -my-2"></div>
