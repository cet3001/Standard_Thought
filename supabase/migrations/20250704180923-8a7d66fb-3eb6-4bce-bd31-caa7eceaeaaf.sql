-- Add data constraints and fix RLS policies
...
-- 4. Add unique constraint to prevent duplicate active subscriber emails
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'unique_active_email'
    ) THEN
        -- Create unique index instead of exclusion constraint for better compatibility
...
-- 5. Add index on subscribers unsubscribe_token for faster lookups
